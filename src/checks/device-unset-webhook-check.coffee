  ...
  check: (callback) =>
    @_getWebhook (error, webhook) =>
      ...
      @_getDevice (error, device) =>
        return callback error if error?
        return callback Errors.DEVICE_WEBHOOK_EXISTS() if _.find device.meshblu?.forwarders?.message?.received, webhook
        callback()
...
module.exports = DeviceUnsetWebhookCheck
